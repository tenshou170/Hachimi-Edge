...
extern "C" fn GetRaceDynamicCameraSettingData(boot_mode: *mut Il2CppObject) -> bool {
    if Hachimi::instance().config.load().force_allow_dynamic_camera {
        ...
        get_orig_fn!(
            ...
            GetRaceDynamicCameraSettingDataFn
        )(boot_mode)
    }
}
...
pub fn init(umamusume: *const Il2CppImage) {
    ...
    let GetRaceDynamicCameraSettingData_addr =
        get_method_addr(RaceUtil, c"GetRaceDynamicCameraSettingData", 1);

    new_hook!(
        GetRaceDynamicCameraSettingData_addr,
        GetRaceDynamicCameraSettingData
    );
}
