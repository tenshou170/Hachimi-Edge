...
use crate::{
    ...
    il2cpp::{
        hook::umamusume::{StoryTimelineController, StoryTimelineData},
...
fn on_http_request(request: &mut Request) -> Result<CommandResponse, Error> {
    ...
    match command {
        Command::StoryGotoBlock {
            block_id,
            incremental,
        } => {
            if block_id < -1 {
                return Ok(CommandResponse::error(
                    "Block ID cannot be smaller than -1".to_owned(),
                ));
            }
            ...
            *params = (block_id, incremental);
            ...
            Thread::main_thread().schedule(|| {
                ...
                fn exec(block_id: i32, incremental: bool) -> i32 {
                    ...
                    let Some(block_list) =
                        <IList>::new(StoryTimelineData::get_BlockList(timeline_data))
                    else {
                        return -3;
                    };
                    ...
                    if incremental && block_id != -1 {
                        ...
                        let start = if last_block_id > block_id {
                            0
                        } else {
                            last_block_id + 1
                        };
                        ...
                        StoryTimelineController::GotoBlock(
                            controller, block_id, false, false, false,
                        );
                    }
                    -2
                }
